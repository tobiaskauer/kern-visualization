...
// src/channels/index.ts
var { CHANNEL_OPTIONS: qo, CONFIG_TYPE: Bo } = Wo, Os = z;
function Cs({ page: r, extraTransports: e = [] }) {
  let t = [new Z({ page: r }), ...e];
  if (Bo === "DEVELOPMENT") {
    let o = window.location.protocol === "http:" ? "ws" : "wss", { hostname: p, port: a } = window.location, { wsToken: c } = qo || {}, i = `${o}\
://${p}:${a}/storybook-server-channel?token=${c}`;
    t.push(new ee({ url: i, onError: /* @__PURE__ */ s(() => {
    }, "onError"), page: r }));
  }
